...
const COLUMNAS_AGREGADAS = [
  ...
  ['servicios', 'turnos_detalle_json', "TEXT NOT NULL DEFAULT '{}'"],
  ['servicios', 'asesores_json', "TEXT NOT NULL DEFAULT '[]'"],
  ['aperturas', 'asesores_json', "TEXT NOT NULL DEFAULT '[]'"],
  ['partidas_precio', 'sueldo', 'REAL'],
  ['aperturas', 'turnos_detalle_json', "TEXT NOT NULL DEFAULT '{}'"],
...
function migrar(db, schema) {
  ...
  if (defCatalogos && !(defCatalogos.sql.includes('motivo_baja') && defCatalogos.sql.includes('turno_dia'))) {
    db.pragma('foreign_keys = OFF');
    db.pragma('legacy_alter_table = ON');
    const columnas = db
  ...
  // Los turnos del día —matutino, nocturno, mixto—, que son la otra mitad de
  // la pregunta. Hasta ahora la plataforma llamaba «turno» a la jornada, y al
  // revisar el sistema se pidió justamente lo que faltaba: a qué hora se cubre.
  if (db.prepare(`SELECT COUNT(*) AS n FROM catalogos WHERE tipo = 'turno_dia'`).get().n === 0) {
    const t = db.prepare(`INSERT OR IGNORE INTO catalogos (tipo, valor, orden) VALUES ('turno_dia',?,?)`);
    ['MATUTINO', 'NOCTURNO', 'MIXTO'].forEach((v, i) => t.run(v, i));
  }

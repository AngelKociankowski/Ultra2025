...
export function partidasDe(servicioId) {
  return getDb()
    ...
    .map((p) => ({
      ...p,
      importe: redondear(p.cantidad * p.precio_unitario),
      // Lo que cuesta la nómina de ese renglón, y lo que deja. Null cuando el
      // sueldo no se ha capturado: cero diría que no cuesta nada, que es peor
      // que no decir nada.
      costo: p.sueldo === null || p.sueldo === undefined ? null : redondear(p.cantidad * p.sueldo),
      margen:
        p.sueldo === null || p.sueldo === undefined
          ? null
          : redondear(p.cantidad * (p.precio_unitario - p.sueldo)),
    }));
}
...
export function resumenDe(servicio) {
  ...
  // La nómina solo se suma con los renglones que tienen sueldo capturado, y se
  // dice cuántos faltan. Sumar los que no lo tienen como si costaran cero daría
  // un margen inventado, y un margen inventado se ve igual de bien que uno real.
  const conSueldo = partidas.filter((p) => p.costo !== null);
  const nomina = redondear(conSueldo.reduce((a, p) => a + p.costo, 0));
  const cubiertoPorSueldo = redondear(conSueldo.reduce((a, p) => a + p.importe, 0));
  ...
  return {
    ...
    nomina,
    // El margen de lo que sí tiene sueldo capturado. Se compara contra el
    // precio de esos mismos renglones, no contra el total del servicio.
    margen: redondear(cubiertoPorSueldo - nomina),
    pctMargen: cubiertoPorSueldo > 0 ? Math.round(((cubiertoPorSueldo - nomina) / cubiertoPorSueldo) * 1000) / 10 : null,
    sinSueldo: partidas.length - conSueldo.length,
...
function normalizar(fila, indice) {
  ...
  // El sueldo es opcional. Hay servicios cotizados de los que todavía no se
  // sabe con qué sueldo se van a cubrir, y exigirlo frenaría la captura del
  // precio, que es lo que sí se sabe al cerrar la venta.
  const sueldo = limpiarNumero(fila.sueldo);
  if (sueldo !== null && sueldo < 0) {
    throw new ValidacionError(`El sueldo del renglón ${indice + 1} no puede ser negativo.`);
  }
  ...
  return {
    ...
    sueldo: sueldo === null ? null : redondear(sueldo),
...
/**
 * Revisa un desglose sin guardarlo, y devuelve los renglones útiles.
 *
 * Existe para que la apertura pueda rechazar un desglose mal capturado ANTES de
 * crear el servicio. Sin esto, el alta se hacía y el desglose fallaba después:
 * la pantalla decía «error» y el servicio ya estaba en el estado de fuerza, que
 * es la peor combinación posible —quien captura cree que no pasó nada y vuelve
 * a intentarlo—.
 */
export function revisarPartidas(filas) {
  const lista = (Array.isArray(filas) ? filas : []).filter(
    (f) => f && (f.puesto || f.cantidad || f.precio_unitario || f.sueldo)
  );
...
export function guardarPartidas(servicioId, filas, usuario) {
  ...
  const lista = (Array.isArray(filas) ? filas : [])
    ...
    .filter((f) => f && (f.puesto || f.cantidad || f.precio_unitario || f.sueldo))
    .map(normalizar);
  ...
  const tx = db.transaction(() => {
    ...
    const ins = db.prepare(
      `INSERT INTO partidas_precio (servicio_id, puesto, turno, cantidad, precio_unitario, sueldo, nota, orden)
       VALUES (?,?,?,?,?,?,?,?)`
    );
    for (const p of lista) {
      ins.run(id, p.puesto, p.turno, p.cantidad, p.precio_unitario, p.sueldo, p.nota, p.orden);
    }

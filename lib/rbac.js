...
export const GRUPOS_CAMPOS = {
  ...
  /**
   * Quién lleva el servicio, también de operaciones.
   *
   * El supervisor y el gerente a cargo no son un dato administrativo: son la
   * asignación del día a día, y cambian cuando alguien sale de vacaciones,
   * cuando se reparte una zona o cuando entra un supervisor nuevo. Quien lo
   * decide es operaciones. Tenerlo cerrado al administrador significaba que
   * cada cambio de ruta tenía que pedirse por fuera, y mientras tanto la
   * plataforma seguía enseñando al supervisor anterior —que es justo el dato
   * que se consulta cuando hay una incidencia y hay que llamarle a alguien.
   *
   * Va en su propio grupo y no dentro de «datos operativos» por lo mismo que el
   * REPSE y la nómina: ese bloque trae precios, sueldos y bonos, y abrirlo
   * entero para poder reasignar un supervisor sería regalar mucho más de lo que
   * se pidió. El asesor NO está aquí: ese es de ventas, y quién atiende
   * comercialmente a un cliente no lo decide operaciones.
   */
  mando: {
    etiqueta: 'Supervisión a cargo',
    roles: ['admin', 'operaciones'],
    campos: ['supervisor', 'gerente'],
  },
  operativo: {
...
// `tipo_repse`, `supervisor` y `gerente` no se repiten en «operativo»: un campo
// en dos grupos se acabaría editando desde dos formularios distintos con reglas
// distintas. El administrador los alcanza por los grupos de arriba, que también
// son suyos.
 ...
 *
 * Operaciones entra por lo que opera: el nombre con el que se conoce el
 * servicio, cuántos guardias tiene, cómo se reparten por jornada y desde cuándo
 * está dado de alta. Es quien detecta esos errores —son los suyos— y hasta
 * ahora tenía que pedirle a un administrador que los arreglara, que es como se
 * llega a que nadie los arregle. Se le deja fuera el `estatus`, porque
 * reactivar un servicio dado de baja no es corregir una captura sino deshacer
 * una cancelación con folio, y la razón social, que es de jurídico.
 *
 * Toda corrección avisa a quien capturó y al área del dato: el permiso se
 * amplía junto con el rastro, no en lugar de él.
 */
export const CORRECCION_POR_ROL = {
  admin: CAMPOS_CORREGIBLES,
  juridico: ['razon_social'],
  operaciones: ['servicio', 'total_guardias', 'turnos', 'fecha_alta'],
};

/**
 * Qué área es dueña de cada campo, para saber a quién avisarle al corregirlo.
 *
 * No es lo mismo que quién puede corregirlo. El administrador puede corregir la
 * razón social, y aun así quien tiene que enterarse es jurídico: es su dato y es
 * quien va a notar si tres correcciones seguidas apuntan a que el contrato dice
 * otra cosa.
 */
export const AREA_DEL_CAMPO = {
  servicio: ['operaciones'],
  total_guardias: ['operaciones'],
  turnos: ['operaciones'],
  fecha_alta: ['operaciones'],
  razon_social: ['juridico'],
  estatus: ['operaciones'],
};

/** Los roles a los que hay que avisar cuando se corrigen estos campos. */
export function areasDe(campos) {
  const areas = new Set();
  for (const c of campos) for (const rol of AREA_DEL_CAMPO[c] || []) areas.add(rol);
  return [...areas];
}
...
export const PERMISOS = {
  ...
  operaciones: ['ver', 'comentar', 'apertura', 'cancelacion', 'corregir'],
...
export function navegacion(rol) {
  // `icono` es el nombre de un trazo de components/Icono.js, no un carácter.
  // Un emoji lo dibuja el sistema operativo y sale distinto en cada máquina;
  // además no hereda el color del texto, así que en la pestaña activa —fondo
  // rojo, texto blanco— se quedaba pintado de su propio color.
  const items = [
    { href: '/', etiqueta: 'Tablero', icono: 'tablero', permiso: 'ver' },
    { href: '/estado-fuerza', etiqueta: 'Estado de fuerza', icono: 'escudo', permiso: 'ver' },
    { href: '/aperturas', etiqueta: 'Aperturas', icono: 'alta', permiso: 'ver' },
    { href: '/cancelaciones', etiqueta: 'Cancelaciones', icono: 'baja', permiso: 'ver' },
    { href: '/cobranza', etiqueta: 'Cobranza', icono: 'dinero', permiso: 'editar_finanzas' },
    ...
    { href: '/juridico', etiqueta: 'Jurídico', icono: 'balanza', permiso: 'ver' },
    ...
    { href: '/precios', etiqueta: 'Precios', icono: 'etiqueta', permiso: 'precios', grupo: 'admin' },
    { href: '/bitacora', etiqueta: 'Bitácora', icono: 'bitacora', permiso: 'bitacora', grupo: 'admin' },
    { href: '/catalogos', etiqueta: 'Catálogos', icono: 'catalogos', permiso: 'catalogos', grupo: 'admin' },
    { href: '/usuarios', etiqueta: 'Usuarios', icono: 'usuarios', permiso: 'usuarios', grupo: 'admin' },
    { href: '/respaldos', etiqueta: 'Respaldos', icono: 'respaldo', permiso: 'respaldos', grupo: 'admin' },
  ];

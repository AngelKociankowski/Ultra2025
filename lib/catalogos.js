 ...
 */
export const ESTADOS_BASE = ['CDMX', 'EDOMEX', 'HGO', 'MOR', 'QRO', 'PUE', 'TLAX', 'GRO'];

/** Las figuras del REPSE con las que se presta el servicio. */
export const REPSE_BASE = ['VIGILANCIA', 'SEGURIDAD PRIVADA', 'MONITOREO', 'CUSTODIA', 'NO APLICA'];

/** Los uniformes de dotación. El administrador ajusta la lista a la suya. */
export const UNIFORMES_BASE = ['ADMINISTRATIVO', 'INDUSTRIAL', 'COMERCIAL', 'CORPORATIVO', 'TÁCTICO', 'GALA'];
...
function sembrar(db, tipos) {
  ...
  db.transaction(() => {
    ...
    if (tipos.has('gerente')) for (const v of distintos('gerente')) ins.run('gerente', v, 0);

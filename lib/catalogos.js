...
export const TIPOS = {
  ...
  /*
   * La llave sigue siendo `turno` y la etiqueta dice «Jornadas». No es un
   * descuido: así es como lo nombra la operación, y llamarle turno a esto era
   * la fuente de una confusión real. Al revisar el sistema se pidió «faltan los
   * turnos, que deben de ser Matutino, Nocturno y Mixto», y la única respuesta
   * posible era que lo que la plataforma llamaba turno es otra cosa.
   *
   * La jornada es el patrón de días —12X36, 24 HRS, 8X16 L-V—. El turno es a
   * qué hora se cubre, y vive en el tipo `turno_dia`. Son dos preguntas y hacen
   * falta las dos.
   *
   * La llave no se renombra porque está escrita en la columna `turnos_json` de
   * servicios, aperturas, cancelaciones y en los cortes cerrados. Cambiarla
   * costaría una migración de todo el histórico para arreglar una palabra que
   * solo se ve en el código.
   */
  turno: {
    etiqueta: 'Jornadas',
    singular: 'Jornada',
    ...
    lo: 'la',
    ayuda:
      'El patrón de días que trabaja el guardia: 12X36, 24 HRS, 8X16 L-V. No es lo mismo que el turno —matutino, nocturno o mixto—, que se captura aparte y responde a qué hora se cubre. Solo se aceptan al capturar las que estén en esta lista.',
    ejemplo: '12X12 L-D',
  },
  forma_pago: {
  ...
  tipo_repse: {
    etiqueta: 'REPSE',
    singular: 'Valor de REPSE',
    plural: 'valores de REPSE',
    columna: 'tipo_repse',
    lo: 'lo',
    ayuda: 'Si el cliente nos pide REPSE o no. No es si ya se entregó el del mes: es un requisito del cliente, y por eso son solo dos respuestas. Agregar una tercera aquí lograría que el mismo dato se capturara de dos maneras.',
    ejemplo: 'SÍ',
  },
  turno_dia: {
    etiqueta: 'Turnos',
    ...
    lo: 'lo',
    ayuda:
      'A qué hora del día trabaja el guardia: matutino, nocturno o mixto. Es distinto de la jornada, que es el patrón de días —12X36, 24 HRS—. Dos servicios con la misma jornada pueden ser operaciones muy distintas según a qué hora se cubre, y hasta ahora eso no se capturaba en ningún lado.',
    ejemplo: 'NOCTURNO',
  },
  motivo_baja: {
    etiqueta: 'Motivos de cancelación',
    singular: 'Motivo de cancelación',
    plural: 'motivos de cancelación',
    columna: null,
    lo: 'lo',
    ayuda:
      'Por qué se pierde un servicio. Se escribía a mano y salieron 126 textos distintos para 330 cancelaciones: «FIN DE SERVICIO», «FIN DE CONTRATO», «TERMINO DE SERVICIO» y «CIERRE DE SERVICIO» son lo mismo escrito de cuatro formas, y así no se puede contar nada. El detalle de cada caso se sigue escribiendo aparte.',
    ejemplo: 'FIN DE CONTRATO',
  },
  uniforme: {
    etiqueta: 'Uniformes',
    singular: 'Uniforme',
...
export const TURNOS_BASE = [
  '48X48', '24X48', '24X24', '24 HRS',
  '13X47', '12X36', '12X24', '12 HRS',
  '12X12 L-D', '12X12 L-S', '12X12 L-V', '12X12 L-J', '12X12 V-D',
  '11X13',
  '10X14', '10X14 L-S', '10X14 L-V',
 ...
 *
 * Cinco columnas de `servicios` —gerente, supervisor, estado, REPSE y
...
/**
 * Zonas que hay que ofrecer aunque todavía no las traiga ningún servicio.
 *
 * Las zonas se siembran con las que ya están capturadas, que es lo correcto
 * para las cuatro geográficas. Pero estas tres no salen de ahí: Toluca es plaza
 * nueva, y «Staff» y «Cubre descansos» no son un lugar sino la manera de
 * agrupar al personal que no está asignado a un punto fijo. Ninguna de las tres
 * puede aparecer sola en la lista, porque no se puede capturar una zona que no
 * existe todavía: sin sembrarlas, la operación no tiene dónde ponerlas.
 */
export const ZONAS_BASE = ['TOLUCA', 'STAFF', 'CUBRE DESCANSOS'];

/**
 * Por qué se pierde un servicio, en dieciséis respuestas.
 *
 * Salieron de agrupar los 125 textos distintos que la operación escribió a mano
 * en 330 cancelaciones. No están inventadas: cada una junta lo que ya se venía
 * diciendo con otras palabras. «FIN DE SERVICIO», «TERMINO DE SERVICIO»,
 * «CIERRE DE SERVICIO» y «Término de proyecto» son el mismo hecho; separados en
 * cuatro renglones no se pueden sumar, y sumarlos es justamente para lo que
 * sirve preguntar el motivo.
 *
 * El corte está pensado para que el reporte responda la pregunta que importa:
 * de lo que se perdió, cuánto fue por nosotros —cobertura, servicio— y cuánto
 * por el cliente —presupuesto, cierre, cambio de proveedor—. Por eso «FALLAS DE
 * COBERTURA» va aparte de «INCONFORMIDAD CON EL SERVICIO» aunque las dos sean
 * culpa nuestra: se arreglan en áreas distintas.
 *
 * Dieciséis y no cuarenta a propósito: una lista que no cabe en una pantalla se
 * elige mal, y vuelve a pasar lo de los 125 textos por otro camino. El caso
 * particular de cada cancelación se sigue escribiendo en el detalle, que no se
 * pierde y es donde vive lo que ninguna lista puede prever.
 *
 * No está «lo pidió el cliente»: eso no es un motivo, es repetir que se
 * canceló. Cuando el cliente no da razón va en «OTRO» con el detalle escrito,
 * que al menos deja ver cuántas veces nos quedamos sin saber por qué.
 */
/**
 * Los turnos del día. Tres, y no es una lista que crezca.
 *
 * La operación los pidió por su nombre: matutino, nocturno y mixto. Van
 * separados de las jornadas porque son otra pregunta: la jornada dice el patrón
 * de días —12X36, 24 HRS, 8X16 L-V— y el turno dice a qué hora se cubre. Dos
 * servicios con la misma jornada pueden ser operaciones muy distintas según eso,
 * y lo que cuesta cubrir la noche no es lo que cuesta cubrir la mañana.
 *
 * Que sean tres y estén cerrados es lo que hace que el dato sirva: en cuanto
 * alguien pueda escribir «nocturno (fines de semana)», vuelve a pasar lo de los
 * motivos de cancelación. Si hace falta ese matiz, es una jornada nueva.
 */
export const TURNOS_DIA_BASE = ['MATUTINO', 'NOCTURNO', 'MIXTO'];

export const MOTIVOS_BAJA_BASE = [
  'FIN DE CONTRATO',
...
/*
 * El REPSE —«SÍ» y «NO», más los cuatro niveles con los que arrancó, ya
 * retirados— no se siembra aquí sino en la migración de `db.js`, junto al
 * arreglo que convierte los datos viejos. Es por orden de arranque: esta
 * siembra solo corre para los tipos que no tienen ni una opción, y cuando
 * llega, la migración ya dejó las dos respuestas puestas. Tenerlo partido en
 * dos lados dejaba los niveles retirados fuera de toda base nueva.
 */

/**
 * Los uniformes y los gerentes: los de la hoja.
 *
...
function sembrar(db, tipos) {
  ...
  db.transaction(() => {
    if (tipos.has('zona')) for (const v of new Set([...distintos('zona'), ...ZONAS_BASE])) ins.run('zona', v, 0);
    ...
    if (tipos.has('motivo_baja')) MOTIVOS_BAJA_BASE.forEach((v, i) => ins.run('motivo_baja', v, i));
    if (tipos.has('turno_dia')) TURNOS_DIA_BASE.forEach((v, i) => ins.run('turno_dia', v, i));
    ...
    // El REPSE no aparece aquí: lo siembra la migración, porque los niveles
    // retirados tienen que entrar aunque el tipo ya traiga las dos respuestas
    // —y esta siembra solo corre cuando el tipo está completamente vacío—.
    // REPSE_BASE y REPSE_RETIRADOS siguen siendo la lista de referencia.
...
export function opciones() {
  ...
  return {
    ...
    motivosBaja: de('motivo_baja'),
    turnosDia: de('turno_dia'),

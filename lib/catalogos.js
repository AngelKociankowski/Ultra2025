...
function sembrar(db) {
  ...
  db.transaction(() => {
    for (const v of distintos('zona')) ins.run('zona', v, 0);
    for (const v of distintos('asesor')) ins.run('asesor', v, 0);
    turnos.forEach((v, i) => ins.run('turno', v, i));
  })();
}
 ...
 *
 * La siembra corre una sola vez, cuando la tabla está vacía: al instalar sobre
 * una base que ya tenía servicios cargados. Si el administrador después deja el
 * catálogo como quiere, nadie se lo vuelve a llenar.
 */
function base() {
  const db = getDb();
  if (!sembrado) {
    if (db.prepare('SELECT COUNT(*) AS n FROM catalogos').get().n === 0) sembrar(db);
    sembrado = true;
  }
  return db;
}
...
export function opciones() {
  ...
  const de = (tipo) =>
    db
      ...
      .all(tipo)
      .map((r) => r.valor);
  return { zonas: de('zona'), asesores: de('asesor'), turnos: de('turno') };
}

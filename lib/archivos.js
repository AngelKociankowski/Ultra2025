...
function extensionDe(archivo) {
  ...
  throw new ValidacionError('Solo se aceptan archivos PDF o XML (el CFDI y su representación impresa).');
}

/** Guarda el archivo y devuelve con qué quedó registrado en la factura. */
export async function guardar(archivo, prefijo) {
  ...
  return {
    ...
    archivo_nombre: String(archivo.name || `factura.${ext}`).replace(/[\r\n"]/g, '').slice(0, 120),

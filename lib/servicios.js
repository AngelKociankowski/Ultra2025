...
import { filtrarCampos, camposCorregibles, CAMPOS_CORREGIBLES, areasDe } from './rbac';
import { avisarCorreccion } from './avisos';
import { turnosAceptados, exigirDelCatalogo, canonicoDelCatalogo, opciones } from './catalogos';
...
import { guardarPartidas, revisarPartidas } from './partidas';
...
/**
 * Deja el desglose por turno diciendo la verdad después de mover las jornadas.
 *
 ...
 * invariante que acababa de exigir:
 *
 *   · Un incremento subía la jornada a 6 y el desglose seguía diciendo 4.
 *   · Una reducción la bajaba a 4 y el desglose seguía diciendo 6.
 *   · Una cancelación total dejaba el servicio en cero guardias y el desglose
 *     seguía afirmando que tenía tres mixtos.
 *   · Una corrección de la plantilla no lo tocaba.
 *   · Una corrección que quitaba una jornada dejaba su desglose huérfano,
 *     apuntando a una jornada que el servicio ya no tenía.
 *
 * El reporte de guardias por turno del tablero suma ese desglose, así que
 * contaba gente que no existe. Nada fallaba, nada se veía raro: simplemente el
 * número estaba mal.
 *
 * La regla, en un solo lugar en vez de en los cinco sitios que mueven jornadas:
 *
 *   1. Se aplica lo que traiga el movimiento, si trae algo.
 *   2. Se tira el desglose de las jornadas que el servicio ya no tiene.
 *   3. Se tira el desglose de la jornada que dejó de cuadrar.
 *
 * Tirarlo es deliberado, y es la parte discutible. La alternativa sería
 * repartir la diferencia a prorrata, y eso sería inventar: si a un servicio de
 * cuatro matutinos le suben dos guardias y nadie dijo de qué turno, la
 * plataforma no sabe si son de mañana o de noche, y escribir una respuesta
 * plausible es peor que no tener ninguna. Un dato viejo con aspecto de bueno se
 * cuela en un reporte; un hueco se ve y se llena.
 *
 * Quien no quiera perderlo lo captura en el movimiento, que es un renglón más
 * en el formulario que ya existe.
 */
function conciliarDetalle(detalleActual, turnosFinales, aporte, signo = 1) {
  const out = {};
  for (const [jornada, reparto] of Object.entries(detalleActual || {})) {
    out[jornada] = { ...(reparto || {}) };
  }

  // 1. Lo que el movimiento sí dijo.
  for (const [jornada, reparto] of Object.entries(aporte || {})) {
    const acc = { ...(out[jornada] || {}) };
    for (const [turno, n] of Object.entries(reparto || {})) {
      const v = (Number(acc[turno]) || 0) + signo * (Number(n) || 0);
      if (v > 0) acc[turno] = v;
      else delete acc[turno];
    }
    if (Object.keys(acc).length) out[jornada] = acc;
    else delete out[jornada];
  }

  // 2 y 3. Fuera lo que ya no describe al servicio.
  for (const jornada of Object.keys(out)) {
    const meta = Number(turnosFinales?.[jornada]) || 0;
    const suma = Object.values(out[jornada]).reduce((a, b) => a + (Number(b) || 0), 0);
    if (!meta || suma !== meta) delete out[jornada];
  }
...
export function listarServicios({ estatus, zona, asesor, turno, contrato, facturado, modalidad, q } = {}) {
  ...
  // El filtro alcanza también a los servicios donde esa persona es el segundo
  // asesor: si solo mirara la columna, «ver lo de Armando» dejaría fuera justo
  // los que comparte, que son los que más fácil se pierden de vista.
  if (asesor) {
    where.push('(asesor = ? OR asesores_json LIKE ?)');
    args.push(asesor, `%"${asesor}"%`);
  }
...
/**
 * El orden de la lista: alfabético por nombre de servicio.
 *
 * Antes se ordenaba por zona y luego por nombre. Dentro de cada zona quedaba
 * alfabético, pero para quien busca un servicio en la pantalla completa el
 * efecto era el contrario: la A aparecía cuatro veces, una por zona, y para
 * encontrar «CHRYSLER» había que saber de antemano en qué zona estaba. La zona
 * ya se filtra con su propio control, así que como orden no aportaba nada.
 *
 * Se ordena en JavaScript y no en SQL a propósito. SQLite compara byte por
 * byte: «ÁLVARO» se iría después de la Z, y «PROQUIGAMA 107» antes que
 * «PROQUIGAMA 67B» porque compara el «1» contra el «6» como texto. Con las
 * reglas del español y `numeric` los dos casos salen como los espera quien lee.
 *
 * Los cancelados van al final —así los deja el ORDER BY de arriba, que es lo
 * único que sigue haciendo SQLite— porque lo que se opera es lo activo.
 */
const COMPARADOR = new Intl.Collator('es', { numeric: true, sensitivity: 'base' });

function porNombre(a, b) {
  if (a.estatus !== b.estatus) return a.estatus < b.estatus ? -1 : 1;
  return COMPARADOR.compare(a.servicio || '', b.servicio || '');
}
...
function hidratar(row) {
  return {
    ...row,
    turnos: JSON.parse(row.turnos_json || '{}'),
    // Vacío en la inmensa mayoría: es un dato que se empezó a pedir después de
    // que ya había 217 servicios capturados. Se devuelve siempre como objeto
    // para que la pantalla no tenga que distinguir «no lo tiene» de «no vino».
    turnosDetalle: JSON.parse(row.turnos_detalle_json || '{}'),
    // Siempre una lista, aunque el servicio tenga un solo asesor: así la
    // pantalla no tiene que distinguir «uno» de «varios» en cada renglón.
    asesores: (() => {
      const l = JSON.parse(row.asesores_json || '[]');
      return l.length ? l : row.asesor ? [row.asesor] : [];
    })(),
  };
}
...
export function registrarApertura(payload, usuario) {
  ...
  // A qué hora se cubre cada jornada. Opcional: los servicios que ya estaban no
  // lo tienen y no se les puede exigir en retrospectiva.
  const turnosDetalle = normalizarTurnosDetalle(payload.turnos_detalle, turnos);
  /**
   * El desglose por puesto y sueldo se REVISA aquí y se guarda al final.
   *
   * Revisarlo antes de crear nada es lo que evita el peor de los estados: que
   * el alta se haga, el desglose falle después, y la pantalla diga «error»
   * mientras el servicio ya está en el estado de fuerza. Quien captura creería
   * que no pasó nada y lo volvería a intentar.
   *
   * Solo en un servicio nuevo. En un incremento las partidas ya existen y
   * fusionarlas —¿se suma al renglón que está, o entra uno nuevo?— es una
   * decisión de quien lleva el servicio, no de un formulario de alta.
   */
  const partidas = tipo === 'INCREMENTO' ? [] : revisarPartidas(payload.partidas);
  // El primero manda en la columna `asesor`; la lista completa va aparte.
  const asesores = normalizarAsesores(payload.asesores, asesor);
  const asesorPrincipal = asesores[0] || asesor;
  ...
  const tx = db.transaction(() => {
    ...
    const info = db
      .prepare(
        `INSERT INTO aperturas
          (folio, tipo, servicio, razon_social, direccion, zona, cluster, estado_geo, asesor, gerente,
           supervisor, reporta, asesores_json,
           guardias, turnos_json, turnos_detalle_json, fecha, periodo, precio_guardia, sueldo_base, bono, uniforme,
      ...
      .run(
        ...
        payload.cluster || null, estadoGeo, asesorPrincipal,
        gerente, supervisor, payload.reporta || usuario?.nombre || null,
        JSON.stringify(asesores),
        guardias, JSON.stringify(turnos), JSON.stringify(turnosDetalle), fecha, periodoDe(fecha),
    ...
    if (tipo === 'INCREMENTO') {
      ...
      const nuevoDetalle = conciliarDetalle(
        JSON.parse(s.turnos_detalle_json || '{}'),
        nuevosTurnos,
        turnosDetalle,
        +1
      );
      db.prepare(
        `UPDATE servicios
            SET total_guardias = total_guardias + ?, turnos_json = ?, turnos_detalle_json = ?,
                actualizado_en = datetime('now')
          WHERE id = ?`
      ).run(guardias, JSON.stringify(nuevosTurnos), JSON.stringify(nuevoDetalle), servicioIdDestino);
      servicioId = servicioIdDestino;
    } else {
      const ins = db
        .prepare(
          `INSERT INTO servicios
             (servicio, razon_social, direccion, zona, tipo, cluster, estado_geo, asesor, asesores_json, gerente,
              supervisor, estatus, modalidad, fecha_fin_prevista,
              total_guardias, turnos_json, turnos_detalle_json, precio_guardia, sueldo_base, bono, uniforme,
        ...
        .run(
          ...
          asesorPrincipal, JSON.stringify(asesores), gerente, supervisor, modalidad, finPrevisto,
          guardias, JSON.stringify(turnos), JSON.stringify(turnosDetalle),
          numOrNull(payload.precio_guardia), numOrNull(payload.sueldo_base), numOrNull(payload.bono),
  ...
  // Ya revisado arriba, así que aquí no puede fallar por captura. Va fuera de
  // la transacción porque `guardarPartidas` abre la suya —recalcula el importe
  // del servicio y deja su propio registro en la bitácora— y better-sqlite3 no
  // anida transacciones.
  if (partidas.length) guardarPartidas(salida.servicioId, partidas, usuario);
...
export function aplicarApertura(aperturaId, usuario, correcciones = {}) {
  ...
  const tx = db.transaction(() => {
    ...
    if (gemelo) {
      ...
      const turnosGemelo = mezclarTurnos(JSON.parse(s.turnos_json || '{}'), turnos, +1);
      db.prepare(
        `UPDATE servicios
            SET total_guardias = total_guardias + ?, turnos_json = ?, turnos_detalle_json = ?,
                actualizado_en = datetime('now')
        ...
        guardias,
        JSON.stringify(turnosGemelo),
        JSON.stringify(
          conciliarDetalle(
            JSON.parse(s.turnos_detalle_json || '{}'),
            turnosGemelo,
            JSON.parse(ap.turnos_detalle_json || '{}'),
            +1
          )
        ),
        gemelo.id
...
export function deshacerApertura(aperturaId, { motivo } = {}, usuario) {
  ...
  const tx = db.transaction(() => {
    ...
    if (s.apertura_id === ap.id) {
      ...
      const turnosTrasDeshacer = mezclarTurnos(JSON.parse(s.turnos_json || '{}'), turnos, -1);
      db.prepare(
        `UPDATE servicios
            SET total_guardias = total_guardias - ?, turnos_json = ?, turnos_detalle_json = ?,
                actualizado_en = datetime('now')
        ...
        guardias,
        JSON.stringify(turnosTrasDeshacer),
        JSON.stringify(
          conciliarDetalle(
            JSON.parse(s.turnos_detalle_json || '{}'),
            turnosTrasDeshacer,
...
export function registrarCancelacion(payload, usuario) {
  ...
  // El motivo sale de la lista y el detalle se escribe. Antes era todo texto
  // libre y dio 125 formas de decir dieciséis cosas, que es lo mismo que no
  // haber preguntado. El detalle es donde sigue viviendo lo que ninguna lista
  // puede prever, y por eso no se recorta ni se valida contra nada.
  const motivo = exigirDelCatalogo('motivo_baja', payload.motivo);
  const motivoDetalle = String(payload.motivo_detalle || '').trim() || null;

  const turnos = normalizarTurnos(payload.turnos);
  // De qué turnos salen los guardias que se retiran. Opcional, igual que al
  // capturar: si no viene, el desglose de esa jornada se retira en vez de
  // quedarse diciendo un número que ya no es cierto.
  const turnosDetalle = normalizarTurnosDetalle(payload.turnos_detalle, turnos);
  ...
  const tx = db.transaction(() => {
      ...
      .prepare(
        `INSERT INTO cancelaciones
           (folio, tipo, servicio, servicio_id, guardias, turnos_json, fecha, periodo, zona, asesor,
            motivo, motivo_detalle, reporta, auditoria, cxc, aut_json, creado_por)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
      )
      .run(
        ...
        motivo, motivoDetalle, payload.reporta || usuario?.nombre || null, payload.auditoria || null,
    ...
    if (tipo === 'CANCELACION') {
      db.prepare(
        `UPDATE servicios
            SET estatus = 'BAJA', total_guardias = 0, turnos_json = '{}', turnos_detalle_json = '{}',
      ...
      db.prepare(
        ...
        JSON.stringify(
          conciliarDetalle(JSON.parse(s.turnos_detalle_json || '{}'), nuevosTurnos, turnosDetalle, -1)
        ),
        servicioId
      );
    }
...
export function registrarCorreccion(id, payload, usuario) {
  ...
  if (turnosJson !== (actual.turnos_json || '{}')) {
    ...
    // Y con ellos el desglose por turno, que si no se queda apuntando a una
    // plantilla que ya no existe —o peor, a una jornada que se acaba de quitar—.
    const detalleAntes = JSON.parse(actual.turnos_detalle_json || '{}');
    const detalleDespues = conciliarDetalle(detalleAntes, turnos, null);
    const detalleJson = JSON.stringify(detalleDespues);
    if (detalleJson !== (actual.turnos_detalle_json || '{}')) {
      sets.turnos_detalle_json = detalleJson;
    }
  }
  ...
  const tx = db.transaction(() => {
    ...
    // Y que vuelva a quien capturó, y al área dueña del dato. La bitácora de
    // arriba deja el registro para auditar; esto es para que la persona que se
    // equivocó se entere, que es lo único que evita que se repita. Va dentro de
    // la transacción a propósito: si la corrección se deshace, el aviso se va
    // con ella.
    const { avisados } = avisarCorreccion(db, {
      servicio: actual,
      cambios,
      motivo,
      usuario,
      areas: areasDe(Object.keys(cambios)),
    });

    return { correccionId: info.lastInsertRowid, cambios, avisados };
  });
...
/**
 * La lista de asesores de un servicio.
 *
 * Un servicio puede llevarlo más de uno, y hasta ahora el campo aceptaba uno
 * solo. La operación lo venía resolviendo como podía: hay dos renglones en el
 * catálogo que son dos personas juntas —«ARMANDO LOPEZ MAURICIO VALENZUELA» y
 * «MAURICIO VALENZUELA/ARMANDO LOPEZ», los mismos dos en distinto orden— y
 * cinco servicios con una barra en medio del nombre. Eso no es un capricho de
 * captura: es lo que pasa cuando el dato no cabe en el campo.
 *
 * Cada nombre se valida contra el catálogo, igual que el asesor de siempre. Se
 * quitan los repetidos conservando el orden, porque el primero es el que se
 * queda en la columna `asesor` y por él filtra media plataforma.
 */
function normalizarAsesores(lista, principal) {
  const out = [];
  const vistos = new Set();
  for (const v of [principal, ...(Array.isArray(lista) ? lista : [])]) {
    const nombre = exigirDelCatalogo('asesor', v);
...
/**
 * El desglose por turno dentro de cada jornada: {"12X36": {"NOCTURNO": 4}}.
 *
 * Es opcional, y esa es la primera regla. La plataforma lleva 217 servicios
 * capturados sin este dato; exigirlo dejaría todos ellos en falta y convertiría
 * cualquier corrección de otra cosa en una recaptura completa. Quien lo tiene,
 * contesta también a qué hora se cubre; quien no, sigue como estaba.
 *
 * La segunda regla es que, cuando está, tiene que cuadrar. Si la jornada dice
 * ocho guardias y el desglose por turno suma seis, el mismo servicio está
 * afirmando dos plantillas distintas, y esa es exactamente la clase de dato que
 * hace que el reparto del tablero no coincida con el estado de fuerza —ya pasó
 * con tres servicios y su desglose por jornada—. Se rechaza en la captura, que
 * es el único momento en que alguien sabe cuál de los dos números es el bueno.
 */
function normalizarTurnosDetalle(detalle, turnos) {
  if (!detalle || typeof detalle !== 'object') return {};
  const aceptados = new Set(opciones().turnosDia);
  const out = {};

  for (const [jornada, reparto] of Object.entries(detalle)) {
    const clave = String(jornada).toUpperCase().trim();
    if (!reparto || typeof reparto !== 'object') continue;
    if (!(clave in turnos)) {
      throw new ValidacionError(
        `El desglose por turno menciona la jornada «${clave}», que este servicio no tiene.`
      );
    }

    const porTurno = {};
    let suma = 0;
    for (const [turno, v] of Object.entries(reparto)) {
      const t = String(turno).toUpperCase().trim();
      const n = Number(v) || 0;
      if (n <= 0) continue;
      if (!aceptados.has(t)) {
        throw new ValidacionError(
          `El turno «${t}» no está en el catálogo. Un administrador lo da de alta en Catálogos.`
        );
      }
      porTurno[t] = n;
      suma += n;
    ...
    if (!suma) continue; // sin capturar: es válido, se deja fuera
    if (suma !== turnos[clave]) {
      throw new ValidacionError(
        `En «${clave}» la jornada dice ${turnos[clave]} guardias y el desglose por turno suma ${suma}. ` +
          'Tienen que coincidir, o el servicio afirma dos plantillas distintas.'
      );
    }
    out[clave] = porTurno;
  }

...
export function aplicarApertura(aperturaId, usuario, correcciones = {}) {
  ...
  const CORREGIBLES = { zona: 'zona', asesor: 'asesor', gerente: 'gerente', supervisor: 'supervisor', estado_geo: 'estado_geo', tipo_repse: 'tipo_repse', uniforme: 'uniforme' };
  const valores = {};
  for (const [campo, tipo] of Object.entries(CORREGIBLES)) {
    valores[campo] = correcciones[campo] !== undefined
      ? exigirDelCatalogo(tipo, correcciones[campo])
      : ap[campo] || null;
  }
  ...
  const tx = db.transaction(() => {
    ...
    const arreglados = Object.keys(CORREGIBLES).filter((c) => correcciones[c] !== undefined && valores[c] !== (ap[c] || null));

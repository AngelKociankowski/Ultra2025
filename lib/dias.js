...
/**
 * Estar en la calle es lo mismo hoy que cualquier otro día.
 *
 * Un servicio cuenta desde su fecha de alta. Suena obvio y no lo era: una
 * apertura registrada hoy con fecha del 28 creaba el servicio al momento y sus
 * guardias empezaban a sumar en el estado de fuerza de inmediato, dos semanas
 * antes de que hubiera nadie parado en esa puerta. El total decía que había
 * gente que todavía no existía, y con ella se facturaba y se calculaba la
 * nómina.
 *
 * Ventas registra la apertura en cuanto cierra —que es lo correcto, así queda
 * el folio y todos la ven venir— y el montaje es después. Entre las dos cosas
 * hay días, y en esos días el servicio está contratado pero no está en la
 * calle.
 *
 * Este fragmento es el que decide. Va aquí, junto a la reconstrucción por día,
 * porque es exactamente la misma regla: «hoy» es una fecha como las demás.
 */
/**
 * La fecha va incrustada en el SQL en vez de ir como parámetro, y conviene
 * decir por qué: better-sqlite3 no deja mezclar parámetros con nombre y
 * posicionales en una misma consulta, y estas los usan posicionales para los
 * filtros. Meterla como `?` obligaría a que cada quien la ponga en el lugar
 * correcto de la lista, que es justo la clase de detalle que se rompe al
 * agregar un filtro nuevo.
 *
 * No hay riesgo de inyección: el valor no viene de nadie. Lo produce
 * `Intl.DateTimeFormat` con la zona de la empresa y siempre sale AAAA-MM-DD.
 * Se afirma de todas formas, porque una suposición que nadie comprueba deja de
 * ser cierta el día que alguien cambia el formateador.
 */
export function yaArranco(dia = hoy()) {
  if (!ES_FECHA.test(dia)) throw new Error(`Fecha interna inesperada: «${dia}».`);
  return `fecha_alta <= '${dia}'`;
}

/** Lo contrario: los que están registrados y todavía no arrancan. */
export function noHaArrancado(dia = hoy()) {
  if (!ES_FECHA.test(dia)) throw new Error(`Fecha interna inesperada: «${dia}».`);
  return `fecha_alta > '${dia}'`;
}

/** Un servicio cuya alta todavía no llega. */
export function porArrancar(s, dia = hoy()) {
  return Boolean(s?.fecha_alta) && s.fecha_alta > dia;
}

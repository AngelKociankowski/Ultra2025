 ...
 *
 * Es la comparación que ningún total hace por sí solo. La suma de las facturas
 * cuadra consigo misma, y la plantilla del estado de fuerza también: cada una
 * es coherente por dentro y las dos pueden estar diciendo cosas distintas sin
 * que nada chille. Una factura por 8 guardias en un servicio de 10 son dos
 * elementos que se están poniendo todos los días y no se cobran; una por 12 en
 * uno de 10 es un cobro que el cliente va a rebotar.
 *
 * Se compara contra dos cosas y por eso hay dos diferencias:
 *
 *   · guardias facturados contra la plantilla del servicio
 *   · importe facturado contra el importe mensual de la ficha, que es el precio
 *     acordado
 *
 * Lo que no tiene con qué compararse se dice, no se cuenta como si cuadrara:
 * las facturas viejas no traen guardias y los servicios sin importe capturado
 * no tienen precio contra el cual medir.
 */
export function conciliacionDelPeriodo(periodo) {
  const p = exigirPeriodo(periodo);
  const filas = getDb()
    .prepare(
      `SELECT f.id, f.concepto, f.importe, f.guardias, f.folio,
              s.id AS servicio_id, s.servicio, s.razon_social, s.zona, s.asesor,
              s.total_guardias, s.importe_factura, s.precio_guardia, s.estatus
         FROM facturas f JOIN servicios s ON s.id = f.servicio_id
        WHERE f.periodo = ? AND f.cancelada = 0
        ORDER BY s.servicio, f.concepto`
    )
    .all(p);

  // Un servicio puede llevar varias facturas del mismo mes —quincenas, extras—
  // y compararlas una por una contra la plantilla entera sería acusar de faltar
  // a cada mitad. Se juntan por servicio y se compara el total.
  const porServicio = new Map();
  for (const f of filas) {
    const acc = porServicio.get(f.servicio_id) || {
      ...
      contratado: Number(f.importe_factura) || 0,
      precio_guardia: Number(f.precio_guardia) || 0,
      facturado: 0,
      guardias: null,
      conceptos: [],
      sinGuardias: 0,
    };
    acc.facturado += Number(f.importe) || 0;
    if (f.guardias === null || f.guardias === undefined) acc.sinGuardias += 1;
    else acc.guardias = (acc.guardias || 0) + Number(f.guardias);
    acc.conceptos.push(f.concepto);
    porServicio.set(f.servicio_id, acc);
  }

  const hallazgos = [];
  const resumen = {
    servicios: porServicio.size,
    cuadran: 0,
    sinGuardias: 0,
    sinPrecio: 0,
    guardiasDeMenos: 0,
  ...
  for (const a of porServicio.values()) {
    // El precio de un guardia: el capturado, o el que se deduce del importe
    // mensual entre la plantilla. Sirve para ponerle pesos a la diferencia.
    const unitario = a.precio_guardia || (a.plantilla > 0 ? a.contratado / a.plantilla : 0);

    const difGuardias = a.guardias === null ? null : a.guardias - a.plantilla;
    const difImporte = a.contratado > 0 ? Math.round((a.facturado - a.contratado) * 100) / 100 : null;

    if (a.sinGuardias > 0) resumen.sinGuardias += 1;
    if (!a.contratado) resumen.sinPrecio += 1;

    const problemas = [];
    if (difGuardias !== null && difGuardias < 0) {
      problemas.push({
        clase: 'guardias_de_menos',
        texto: `Se facturaron ${a.guardias} guardias y el servicio tiene ${a.plantilla}: ${-difGuardias} sin cobrar.`,
        monto: Math.round(-difGuardias * unitario * 100) / 100,
      });
      resumen.guardiasDeMenos += -difGuardias;
    }
    if (difGuardias !== null && difGuardias > 0) {
      problemas.push({
        clase: 'guardias_de_mas',
        texto: `Se facturaron ${a.guardias} guardias y el servicio tiene ${a.plantilla}: ${difGuardias} de más.`,
        monto: Math.round(difGuardias * unitario * 100) / 100,
      });
      resumen.guardiasDeMas += difGuardias;
    }
    // Un peso de diferencia es redondeo; se avisa a partir de uno.
    const importeFuera = difImporte !== null && Math.abs(difImporte) >= 1;
    if (importeFuera) {
      problemas.push({
        clase: difImporte < 0 ? 'importe_de_menos' : 'importe_de_mas',
        texto:
          difImporte < 0
            ? `Se facturaron ${moneda(a.facturado)} y el acuerdo dice ${moneda(a.contratado)}: faltan ${moneda(-difImporte)}.`
            : `Se facturaron ${moneda(a.facturado)} y el acuerdo dice ${moneda(a.contratado)}: ${moneda(difImporte)} de más.`,
        monto: Math.abs(difImporte),
      });
    }

    // El dinero se cuenta UNA vez.
    //
    // Facturar cuatro guardias de menos y facturar $95,608 de menos no son dos
    // pérdidas: son la misma falta contada de dos maneras, y sumarlas daba el
    // doble. Manda la diferencia de importe cuando la hay, porque es dinero
    // medido y no estimado; los guardias solo ponen la cifra cuando no hay
    // precio en la ficha contra el cual comparar.
    const faltante = importeFuera && difImporte < 0
      ? -difImporte
      : !importeFuera && difGuardias !== null && difGuardias < 0
        ? Math.round(-difGuardias * unitario * 100) / 100
        : 0;
    const sobrante = importeFuera && difImporte > 0 ? difImporte : 0;
    resumen.dineroDeMenos += faltante;
    resumen.dineroDeMas += sobrante;

    if (!problemas.length) resumen.cuadran += 1;
    else hallazgos.push({ ...a, difGuardias, difImporte, problemas });
  }

  // Lo más caro primero: es por donde se empieza a revisar.
  hallazgos.sort((a, b) => {
    const peso = (x) => Math.max(...x.problemas.map((p) => p.monto || 0), 0);
    return peso(b) - peso(a);
  });

  resumen.dineroDeMenos = Math.round(resumen.dineroDeMenos * 100) / 100;
  resumen.dineroDeMas = Math.round(resumen.dineroDeMas * 100) / 100;
  return { periodo: p, hallazgos, resumen };
}

const moneda = (v) =>
  Number(v || 0).toLocaleString('es-MX', { style: 'currency', currency: 'MXN', maximumFractionDigits: 0 });

export function facturasDelPeriodo(periodo) {
  const ref = hoy();
  return getDb()
    .prepare(
      `SELECT f.*, s.servicio, s.razon_social, s.zona, s.asesor
         FROM facturas f JOIN servicios s ON s.id = f.servicio_id
        WHERE f.periodo = ?
        ORDER BY s.servicio, f.concepto`
    )
...
export function registrarFactura(payload, usuario) {
  ...
  // Cuántos guardias cubre. Se pide siempre porque es la mitad de la
  // comparación: sin él, «facturé $84,360» no se puede contrastar contra lo que
  // hay en la calle. Se acepta vacío en las cargas históricas, donde no hay de
  // dónde sacarlo.
  const guardias =
    payload.guardias === undefined || payload.guardias === null || payload.guardias === ''
      ? null
      : Math.round(Number(payload.guardias));
  if (guardias !== null && (!Number.isFinite(guardias) || guardias < 0)) {
    throw new ValidacionError('Los guardias facturados deben ser un número mayor o igual a cero.');
  }
  const dias =
  ...
  /**
   * Un servicio puede llevar varias facturas del mismo mes.
   *
   * Antes se bloqueaba la segunda: la comprobación era «servicio + periodo +
   * concepto», y como el concepto de un servicio a mes vencido es siempre «Mes
   * completo», la primera factura cerraba el mes. Cobranza reportó el caso real
   * —clientes que reciben dos, tres y hasta cuatro facturas por el mismo mes,
   * porque se les factura por sede o por centro de costos— y el efecto era peor
   * que la molestia: la única factura que sí entraba quedaba enfrentada al
   * importe completo del servicio, así que el mes se veía cobrado de menos
   * cuando en realidad estaba completo en cuatro papeles.
   *
   * Lo que sí se sigue frenando es registrar dos veces la MISMA factura, que es
   * el accidente de verdad —doble clic, o volver a capturar sin saber que ya
   * estaba—. Se detecta por dos caminos:
   */
  const folio = String(payload.folio || '').trim();

  // 1. El folio fiscal. Es el identificador real de una factura: si ya está
  //    registrado en este servicio y este mes, es la misma.
  if (folio) {
    const mismoFolio = db
      .prepare('SELECT id, concepto FROM facturas WHERE servicio_id = ? AND periodo = ? AND folio = ?')
      .get(servicio.id, periodo, folio);
    if (mismoFolio) {
      throw new ValidacionError(
        `La factura con folio ${folio} ya está registrada en este servicio para ${periodo} (${mismoFolio.concepto}). Si estuvo mal, cancélala antes de volver a emitirla.`
      );
    }
  }

  // 2. Sin folio no hay identificador, así que se compara el contenido. Dos
  //    facturas del mismo servicio, mes, concepto, fecha e importe no son dos
  //    facturas: son la misma capturada dos veces. Un cliente con cuatro
  //    facturas del mes las tiene con importes o conceptos distintos.
  //
  //    Y esta comparación NO corre cuando viene folio. Es el caso que reportó
  //    cobranza y que esta comprobación seguía bloqueando: un cliente al que se
  //    le factura por sede recibe dos facturas del mismo mes, con el mismo
  //    concepto, la misma fecha y el MISMO importe —una por sede—, y son dos
  //    facturas distintas con dos folios distintos. El mensaje de error incluso
  //    decía «captúrale su folio fiscal», pero el código comparaba el contenido
  //    de todas formas: se le pedía a cobranza hacer justo lo que no servía.
  //
  //    Si el folio es distinto, son dos facturas. Eso no es una interpretación
  //    nuestra: es lo que significa un folio fiscal.
  const mismoContenido = folio
    ? null
    : db
        .prepare(
          `SELECT id FROM facturas
            WHERE servicio_id = ? AND periodo = ? AND concepto = ? AND fecha_factura = ? AND importe = ?`
        )
        .get(servicio.id, periodo, concepto, fecha, importe);
  if (mismoContenido) {
    throw new ValidacionError(
      `Ya hay una factura idéntica de ${concepto.toLowerCase()} para ${periodo} en este servicio: misma fecha y mismo importe. ` +
        'Si de verdad son dos facturas distintas, cámbiale el concepto o captúrale su folio fiscal.'
    );
  }
      ...
      .prepare(
        `INSERT INTO facturas
           (servicio_id, periodo, concepto, folio, fecha_factura, dias_credito, fecha_vencimiento,
            importe, guardias, forma_pago, creado_por)
         VALUES (?,?,?,?,?,?,?,?,?,?,?)`
      )
      .run(
        servicio.id, periodo, concepto, folio || null, fecha, dias, vencimiento,
        importe, guardias, payload.forma_pago || servicio.forma_pago || null, usuario?.id ?? null
      );
...
export function pendientesDeFacturar(periodo) {
  ...
  const p = exigirPeriodo(periodo);
  /**
   * Solo lo que ya arrancó.
   *
   * Un servicio contratado para el 28 no prestó nada en agosto, así que
   * proponerle factura de agosto es proponer cobrar un servicio que no se dio.
   * Se cuela solo con que alguien acepte la propuesta sin mirarla, y del otro
   * lado hay un cliente que la va a rebotar.
   */
  const servicios = db
    .prepare(`SELECT * FROM servicios WHERE estatus = 'ACTIVO' AND ${yaArranco()} ORDER BY servicio`)
    .all();
  ...
  /**
   * Lo que ya se le registró a un servicio este mes.
   *
   * Se lee entero —no solo si existe— porque un servicio con su programa
   * cubierto no está necesariamente cobrado completo: puede llevar una factura
   * de 53,019 contra un servicio de 180,000 porque al cliente se le factura por
   * sede y faltan tres papeles más.
   */
  const registradas = db.prepare(
    `SELECT concepto, folio, fecha_factura, importe, guardias
       FROM facturas
      WHERE servicio_id = ? AND periodo = ? AND cancelada = 0
  ...
  for (const s of servicios) {
    ...
    if (!programa.length) {
      ...
      porFacturar.push({
        ...
        zona: s.zona,
        asesor: s.asesor,
        // La plantilla del servicio, para proponerla como guardias facturados.
        // Es lo que está en la calle: si se factura otra cosa, que sea porque
        // alguien la cambió a propósito y no porque el campo llegó vacío.
        plantilla: Number(s.total_guardias) || 0,
    ...
    if (!pendiente) {
      facturados++;
      /**
       * Cubierto no es lo mismo que terminado.
       *
       * Antes estos servicios desaparecían de la pantalla en cuanto tenían su
       * factura, y con ellos la única puerta para registrarle otra al mismo
       * cliente. Se quedan a la vista con lo que llevan y lo que falta contra
       * el importe de su ficha, que es la comparación que cobranza hace de
       * cabeza cuando un cliente recibe cuatro facturas al mes.
       */
      const facturas = registradas.all(s.id, p);
      const sumaImporte = Math.round(facturas.reduce((a, f) => a + (Number(f.importe) || 0), 0) * 100) / 100;
      const contratado = Math.round((Number(s.importe_factura) || 0) * 100) / 100;
      const conGuardias = facturas.filter((f) => f.guardias !== null);
      yaFacturados.push({
        servicio_id: s.id,
        servicio: s.servicio,
        razon_social: s.razon_social,
        ...
        sumaImporte,
        contratado,
        // Sin importe en la ficha no hay contra qué comparar, y un «faltan
        // $0» diría que está completo cuando lo que pasa es que no se sabe.
        faltaImporte: contratado > 0 ? Math.round((contratado - sumaImporte) * 100) / 100 : null,
        sumaGuardias: conGuardias.length ? conGuardias.reduce((a, f) => a + Number(f.guardias), 0) : null,
      });
    }
  }

  return { periodo: p, porFacturar, sinCondiciones, facturados, yaFacturados };
}
 ...
 *
 * Hasta ahora no había manera. Se podía emitir una factura, cobrarla o
 * cancelarla, pero no arreglar un importe mal tecleado: cobranza reportó
 * «el importe no es el correcto, pero no me deja mover», y era literal. Peor
 * todavía, la única vía que parecía servir —mandar el importe nuevo— caía en el
 * registro de pagos, así que la plataforma entendía «me pagaron esto» cuando le
 * estaban diciendo «esto estaba mal escrito».
 *
 * La alternativa que quedaba era cancelar y volver a emitir, y no es lo mismo:
 * deja en el expediente una factura cancelada que nunca lo estuvo, y el papel
 * que el cliente tiene en la mano sigue siendo el mismo. Cancelar es para la
 * factura que no debió existir; esto es para el número que se escribió mal.
 *
 * Tres reglas, y las tres vienen de que esto toca dinero:
 *
 *   1. Exige motivo. Dentro de seis meses, un importe cambiado sin explicación
 *      es indistinguible de un ajuste para cuadrar algo.
 *   2. No puede dejar la factura por debajo de lo ya cobrado. Si entraron
 *      $50,000 contra una factura de $80,000 y alguien la corrige a $30,000, el
 *      saldo se vuelve negativo y el cliente aparece con dinero a favor que
 *      nadie le debe.
 *   3. No toca una factura cancelada. Ya no cuenta para nada; corregirla sería
 *      arreglar la ortografía de algo que se dio por no escrito.
 *
 * El vencimiento se recalcula si cambia la fecha, con los días de crédito que
 * la factura ya tenía guardados: si al cliente le renegociaron el plazo después
 * de emitirla, esta conserva el que era su condición cuando se emitió.
 */
const CAMPOS_FACTURA = ['periodo', 'concepto', 'fecha_factura', 'importe', 'folio', 'guardias'];

export function corregirFactura(id, payload, usuario) {
  const db = getDb();
  const factura = db.prepare('SELECT * FROM facturas WHERE id = ?').get(Number(id));
  if (!factura) throw new ValidacionError('Esa factura no existe.');
  if (factura.cancelada) {
    throw new ValidacionError('Esa factura está cancelada: ya no cuenta para el saldo y no hay nada que corregir.');
  }
  ...
  if (payload.importe !== undefined) {
    const nuevo = Math.round((Number(payload.importe) || 0) * 100) / 100;
    if (!(nuevo > 0)) throw new ValidacionError('El importe de una factura tiene que ser mayor que cero.');
    const pagado = Math.round((Number(factura.importe_pagado) || 0) * 100) / 100;
    if (nuevo < pagado) {
      throw new ValidacionError(
        `Esta factura ya tiene ${formatoMoneda(pagado)} cobrados y la estás dejando en ${formatoMoneda(nuevo)}. ` +
          'El saldo quedaría a favor del cliente. Corrige primero el pago, o cancela la factura y emítela como va.'
      );
    }
    if (nuevo !== Math.round((Number(factura.importe) || 0) * 100) / 100) {
      cambios.importe = { antes: factura.importe, despues: nuevo };
      sets.importe = nuevo;
    }
  }

  if (payload.guardias !== undefined) {
    const n = payload.guardias === null || payload.guardias === '' ? null : Number(payload.guardias);
    if (n !== null && (!Number.isInteger(n) || n < 0)) {
      throw new ValidacionError('Los guardias facturados tienen que ser un número entero.');
    }
    if (n !== (factura.guardias ?? null)) {
      cambios.guardias = { antes: factura.guardias, despues: n };
      sets.guardias = n;
    }
  }

  if (payload.periodo !== undefined) {
    const p = exigirPeriodo(payload.periodo);
    if (p !== factura.periodo) {
      cambios.periodo = { antes: factura.periodo, despues: p };
      sets.periodo = p;
    }
  }

  if (payload.concepto !== undefined) {
    const c = String(payload.concepto || '').trim();
    if (!c) throw new ValidacionError('El concepto no puede quedar vacío.');
    if (c !== factura.concepto) {
      cambios.concepto = { antes: factura.concepto, despues: c };
      sets.concepto = c;
    }
  }

  if (payload.folio !== undefined) {
    const f = String(payload.folio || '').trim() || null;
    if (f && f !== factura.folio) {
      // El folio identifica la factura: dos iguales en el mismo servicio y mes
      // son la misma capturada dos veces, y esa es justo la confusión que se
      // quiere evitar al corregir.
      const choca = db
        .prepare('SELECT id FROM facturas WHERE servicio_id = ? AND periodo = ? AND folio = ? AND id <> ?')
        .get(factura.servicio_id, sets.periodo || factura.periodo, f, factura.id);
      if (choca) {
        throw new ValidacionError(`Ya hay otra factura con el folio ${f} en este servicio y mes.`);
      }
    }
    if (f !== (factura.folio || null)) {
      cambios.folio = { antes: factura.folio, despues: f };
      sets.folio = f;
    }
  }

  if (payload.fecha_factura !== undefined) {
    const nueva = String(payload.fecha_factura || '').trim();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(nueva)) throw new ValidacionError('La fecha de la factura debe venir como AAAA-MM-DD.');
    if (nueva !== factura.fecha_factura) {
      cambios.fecha_factura = { antes: factura.fecha_factura, despues: nueva };
      sets.fecha_factura = nueva;
      // El plazo que corre es el que tenía la factura, no el que el servicio
      // tenga hoy: esa era la condición pactada cuando se emitió.
      sets.fecha_vencimiento = vencimientoDe(nueva, factura.dias_credito);
      cambios.fecha_vencimiento = { antes: factura.fecha_vencimiento, despues: sets.fecha_vencimiento };
    }
  }
  ...
  const tx = db.transaction(() => {
    ...
    db.prepare(`UPDATE facturas SET ${columnas.map((c) => `${c} = ?`).join(', ')} WHERE id = ?`).run(
      ...columnas.map((c) => sets[c]),
      factura.id
    );
    recalcular(db, factura.servicio_id);

    const servicio = db.prepare('SELECT servicio FROM servicios WHERE id = ?').get(factura.servicio_id);
    const detalle = Object.keys(cambios)
      .filter((c) => c !== 'fecha_vencimiento')
      .join(', ');
    auditar(db, {
      usuario,
      accion: 'factura_corregida',
      entidad: 'servicio',
      entidad_id: factura.servicio_id,
      detalle: `${servicio?.servicio || ''} — se corrigió ${detalle} de la factura de ${factura.concepto.toLowerCase()} ${factura.periodo} · ${motivo}`,
      cambios,
    });
  });

  tx();
  return { corregida: true, cambios };
}

const formatoMoneda = (n) =>
  new Intl.NumberFormat('es-MX', { style: 'currency', currency: 'MXN', maximumFractionDigits: 2 }).format(n || 0);

// ------------------------------------------- el número de factura del mes

/**
 * El número de factura de cada servicio en un mes, para la lista del estado de
 * fuerza.
 *
 * Sale de dos lados distintos, y conviene no confundirlos:
 *
 *   - El **libro de facturas** de la plataforma: lo que se emitió aquí. Trae el
 *     folio fiscal y, si se subió, el PDF. Es lo único que se puede abrir.
 *   - El **corte del mes**, columna `factura_mensual`: el número que se anotaba
 *     en la hoja de Excel. Son 6,148 renglones de historia, y no tienen papel
 *     detrás: sirven para saber con qué número se cobró, no para verlo.
 *
 * Se busca en ese orden. Enseñar el número del corte cuando no hay factura en
 * el libro es mejor que un hueco: el dato existe y alguien lo capturó.
 */
export function facturasDelPeriodoPorServicio(periodo) {
  const db = getDb();
  const mapa = new Map();
  if (!periodo) return mapa;
  for (const f of db
    .prepare(
      `SELECT id, servicio_id, folio, importe, importe_pagado, archivo, archivo_nombre, periodo, cancelada
         FROM facturas
        WHERE periodo = ? AND cancelada = 0
        ORDER BY id`
    )
    .all(periodo)) {
    mapa.set(f.servicio_id, f);
  }
  return mapa;
}

/**
 * Los nombres que aparecen una sola vez en `servicios`. Trece se repiten —hay
 * dos «CHRYSLER ANZURES»— y para esos no se puede saber a cuál de los dos
 * pertenece un renglón del corte, así que se quedan fuera en lugar de
 * arriesgarse a enseñar el número del otro.
 */
function nombresSinRepetir(db) {
  return db
    .prepare('SELECT servicio, MIN(id) AS id FROM servicios GROUP BY servicio HAVING COUNT(*) = 1')
    .all();
}

/** Nombre del servicio -> su id, solo para los nombres que no se repiten. */
export function idsPorNombre() {
  const mapa = new Map();
  for (const r of nombresSinRepetir(getDb())) mapa.set(r.servicio, r.id);
  return mapa;
}

/**
 * Los números que quedaron anotados en el corte **de ese mes**, por servicio.
 *
 * El mes importa y no es un detalle: la factura de mayo no es la de agosto. Una
 * columna que enseñe «la última que hubo» convierte cada renglón en una
 * pregunta —¿de cuándo es este número?— y hace que un mes sin facturar parezca
 * facturado. Cada periodo enseña el suyo o no enseña nada.
 */
export function numerosDelCorte(periodo) {
  const db = getDb();
  const mapa = new Map();
  if (!periodo) return mapa;
  const filas = db
    .prepare(
      `SELECT s.id AS servicio_id, sn.factura_mensual AS numero
         FROM servicios s
         JOIN snapshots sn ON sn.servicio = s.servicio
        WHERE sn.periodo = ?
          AND sn.factura_mensual IS NOT NULL AND TRIM(sn.factura_mensual) <> ''
          AND s.servicio IN (SELECT servicio FROM servicios GROUP BY servicio HAVING COUNT(*) = 1)`
    )
    .all(periodo);
  for (const f of filas) mapa.set(f.servicio_id, { numero: String(f.numero).trim(), periodo });
  return mapa;
}

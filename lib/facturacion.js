...
/**
 * El último número de factura que quedó anotado en los cortes, por servicio.
 * Es lo que se enseña mientras el libro nuevo se llena.
 */
export function numerosDelHistorico() {
  const db = getDb();
  const mapa = new Map();
  const filas = db
    .prepare(
      `SELECT s.id AS servicio_id, sn.factura_mensual AS numero, MAX(sn.periodo) AS periodo
         FROM servicios s
         JOIN snapshots sn ON sn.servicio = s.servicio
        WHERE sn.factura_mensual IS NOT NULL AND TRIM(sn.factura_mensual) <> ''
          AND s.servicio IN (SELECT servicio FROM servicios GROUP BY servicio HAVING COUNT(*) = 1)
        GROUP BY s.id`
    )
    .all();
  for (const f of filas) mapa.set(f.servicio_id, { numero: String(f.numero).trim(), periodo: f.periodo });
  return mapa;
}

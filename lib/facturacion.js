...
/**
 * Emite de una sola vez las facturas que le tocan al mes.
 *
 * Es idempotente a propósito: vuelve a pulsarse sin miedo porque salta las que
 * ya existen. Deja fuera lo que no puede calcular —servicios sin esquema o sin
 * importe— y lo devuelve para que finanzas sepa qué le falta capturar en vez de
 * quedarse con la duda de por qué salieron menos facturas de las esperadas.
 */
export function generarDelPeriodo(periodo, usuario) {
  const db = getDb();
  const p = exigirPeriodo(periodo);
  const servicios = db.prepare("SELECT * FROM servicios WHERE estatus = 'ACTIVO' ORDER BY servicio").all();

  const creadas = [];
  const omitidos = { sinEsquema: [], sinImporte: [], yaFacturados: [] };

  const tx = db.transaction(() => {
    for (const s of servicios) {
      if (!s.esquema_facturacion || s.esquema_facturacion === 'SIN_CALENDARIO') {
        omitidos.sinEsquema.push(s.servicio);
        continue;
      }
      if (!(Number(s.importe_factura) > 0)) {
        omitidos.sinImporte.push(s.servicio);
        continue;
      }
      let algunaNueva = false;
      for (const parte of programaDe(s, p)) {
        const existe = db
          .prepare('SELECT id FROM facturas WHERE servicio_id = ? AND periodo = ? AND concepto = ?')
          .get(s.id, p, parte.concepto);
        if (existe) continue;
        db.prepare(
          `INSERT INTO facturas
             (servicio_id, periodo, concepto, fecha_factura, dias_credito, fecha_vencimiento,
              importe, forma_pago, creado_por)
           VALUES (?,?,?,?,?,?,?,?,?)`
        ).run(
          s.id, p, parte.concepto, parte.fecha, parte.dias_credito, parte.fecha_vencimiento,
          Math.round(parte.importe * 100) / 100, s.forma_pago || null, usuario?.id ?? null
        );
        creadas.push({ servicio: s.servicio, concepto: parte.concepto, importe: parte.importe });
        algunaNueva = true;
      }
      if (!algunaNueva) omitidos.yaFacturados.push(s.servicio);
      recalcular(db, s.id);
    }

    auditar(db, {
      usuario,
      accion: 'facturacion_mes',
      entidad: 'facturacion',
      entidad_id: null,
      detalle: `Facturación de ${p}: ${creadas.length} factura(s) emitida(s)`,
    });
  });

  tx();
  const importe = creadas.reduce((a, f) => a + f.importe, 0);
  return { periodo: p, creadas: creadas.length, importe, omitidos };
}

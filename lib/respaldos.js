...
export function crear({ motivo = 'manual', usuario = null, conArchivos } = {}) {
  ...
  try {
    ...
    const baseDatos = fs.readFileSync(temporal);

    const entradas = [{ nombre: 'ultra.db', datos: baseDatos }];
    if (completo) {
      const origen = carpetaArchivos();
      for (const f of fs.readdirSync(origen)) {
        const ruta = path.join(origen, f);
        if (fs.statSync(ruta).isFile()) entradas.push({ nombre: `archivos/${f}`, datos: fs.readFileSync(ruta) });
      }
    }

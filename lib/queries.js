...
export function distribucionPorTurno({ estatus = 'ACTIVO' } = {}) {
  return repartoDeTurnos(
    getDb().prepare(`SELECT id, servicio, turnos_json, total_guardias FROM servicios WHERE estatus = ?`).all(estatus)
  );
}
  ...
  const filas = db
    .prepare(
      `SELECT modalidad,
              ...
              COALESCE(SUM(total_guardias), 0) AS guardias,
              COALESCE(SUM(importe_factura), 0) AS facturacion
         FROM servicios WHERE estatus = 'ACTIVO' GROUP BY modalidad`
    )
    .all();
  ...
  const vencidos = db
    .prepare(
      `SELECT id, servicio, zona, asesor, modalidad, total_guardias, importe_factura, fecha_fin_prevista
         FROM servicios
        WHERE estatus = 'ACTIVO' AND modalidad <> 'FIJO'

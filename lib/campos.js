...
/*
 * Los motivos de cancelación ya no viven aquí: son un catálogo, en la tabla
 * `catalogos` bajo el tipo `motivo_baja`, y la lista de arranque está en
 * `lib/catalogos.js` (MOTIVOS_BAJA_BASE).
 *
 * Estaban escritos en este archivo, diez fijos, y no funcionó por dos motivos
 * que se refuerzan: el administrador no podía agregar uno sin que alguien
 * tocara el código, y la opción «OTRO» reemplazaba el motivo por texto libre.
 * Resultado, 125 motivos distintos para 330 cancelaciones, con el mismo hecho
 * escrito de cuatro formas. Ahora el motivo se elige de la lista —que el
 * administrador mantiene— y el caso particular va en un campo de detalle
 * aparte, donde no estorba para contar.
 */
